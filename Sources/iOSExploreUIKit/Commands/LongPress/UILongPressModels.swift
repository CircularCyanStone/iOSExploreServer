...
public struct UILongPressInput: CommandInput, Sendable, Equatable {
    ...
    public static func parse(decoding decoder: inout CommandInputDecoder) throws -> UILongPressInput {
        let duration = try decoder.read(UIKitActionContracts.uiLongPressDurationField)
        let viewSnapshotID = try decoder.read(UIKitActionContracts.uiLongPressViewSnapshotIDField)
        let locator = try UIKitLocatorInput.parseOptional(
            decoder: &decoder,
            identifierField: UIKitActionContracts.uiLongPressAccessibilityIdentifierField,
            pathField: UIKitActionContracts.uiLongPressPathField
        )
        return UILongPressInput(duration: duration,
                                locator: locator,
