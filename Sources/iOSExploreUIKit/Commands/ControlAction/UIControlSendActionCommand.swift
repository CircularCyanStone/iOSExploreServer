...
struct UIControlSendActionCommand: Command {
    ...
    /// `help` 命令展示的说明。
    let description = "向指定 UIControl 发送 target-action 事件。调用前必须先调 ui.inspect，并把同响应返回的 viewSnapshotID 原样传入；value 字段对 UISlider/UISegmentedControl/UIStepper/UISwitch 有效，缺省则只发事件不改值"
