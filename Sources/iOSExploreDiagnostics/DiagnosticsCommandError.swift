...
/// envelope 文案。
struct DiagnosticsCommandError: Error, Sendable, Equatable {
    ...
    ///   runtime 状态被测试重置。
    static func runtimeNotInstalled(action: String) -> DiagnosticsCommandError {
        DiagnosticsCommandError(failure: ExploreCommandFailure(
            code: .internalError,
    ...
    static func staleCursor(action: String, currentSessionID: String?) -> DiagnosticsCommandError {
        DiagnosticsCommandError(failure: ExploreCommandFailure(
            code: .staleCursor,
