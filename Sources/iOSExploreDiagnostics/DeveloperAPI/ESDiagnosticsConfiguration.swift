...
public struct ESDiagnosticsConfiguration: Sendable, Equatable {
    ...
    /// 是否尝试捕获 `NSLog` 输出；路径为 stderr 行识别与 fishhook 增强 Objective-C 调用点覆盖。
    public let captureNSLog: Bool
    /// 是否尝试通过 `OSLogStore` 读取当前进程的 os_log / Swift Logger entry。
    ///
    ...
    /// 写入前使用的脱敏器。
    public let redaction: ESLogRedactor

    /// 默认配置。
    public static let `default` = ESDiagnosticsConfiguration()
    ...
    public init(captureExploreLogs: Bool = true,
                ...
                redaction: ESLogRedactor = .standard) {
