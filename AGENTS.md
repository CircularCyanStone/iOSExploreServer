...
| Skill | 说明 | 可信度 |
|---|---|---|
| `/ios-automation` | **统一入口**：自动管理 iproxy + 路由到专业 skills + 快速诊断 | ⭐⭐⭐⭐⭐ |
| `/ios-form-filling` | 表单填写、文本输入、开关控制 | ⭐⭐⭐⭐⭐ |
| `/ios-alert-handling` | 弹窗处理、确认对话框 | ⭐⭐⭐⭐⭐ |
| `/ios-navigation` | 页面导航、返回、导航栏按钮 | ⭐⭐⭐⭐⭐ |
| `/ios-list-interaction` | 列表滚动、查找项目 | ⭐⭐⭐⭐⭐ |
...
对 `Examples/SPMExample` 做真实闭环验证时，用启动参数或环境变量让 App 自动调用 `server.start()`：
- `--ios-explore-autostart` 或 `IOS_EXPLORE_AUTOSTART=1` — 自动启动 server
- `--ios-explore-open-alert-test` 或 `IOS_EXPLORE_OPEN_ALERT_TEST=1` — 启动后自动进入弹窗测试页

这些开关是测试工具约定，不是一次性临时状态，后续 agent 应复用同一套开关。
...
不需要 iproxy（模拟器与 Mac 共享 localhost）。
