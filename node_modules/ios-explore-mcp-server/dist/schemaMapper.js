...
export function mapInputSchema(schema) {
    const inputSchema = {};
    const extensionLines = [];
    const requiredFromConstraints = [];
    ...
    if (requiredFromConstraints.length > 0) {
        const existing = Array.isArray(inputSchema.required) ? inputSchema.required : [];
    ...
    let descriptionSuffix = extensionLines.length === 0
        ? ""
...
function flattenTopLevelComposition(inputSchema) {
    const notes = [];
    // oneOf / anyOf：分支表达"互斥二选一"或"多选一"替代项。
    for (const key of ["oneOf", "anyOf"]) {
        if (!Array.isArray(inputSchema[key]))
            continue;
        const alternates = collectAlternatesFromBranches(inputSchema[key], inputSchema);
    ...
    if (Array.isArray(inputSchema.allOf)) {
        const subs = inputSchema.allOf.filter(isPlainObject);
        ...
        for (const sub of subs) {
            ...
            for (const key of ["oneOf", "anyOf"]) {
                if (Array.isArray(sub[key])) {
                    const alternates = collectAlternatesFromBranches(sub[key], inputSchema);
                    if (alternates.length >= 2) {
                        removeFromRequired(inputSchema, alternates);
                        augmentAlternateFields(inputSchema, alternates);
                        notes.push(alternateNote(key, alternates));
...
function collectAlternatesFromBranches(branches, inputSchema) {
...
function alternateNote(key, alternates) {
...
function mergePropertiesInto(inputSchema, source) {
    const srcProps = source.properties;
    if (!isPlainObject(srcProps))
        return;
...
function requiredStrings(schema) {
    return Array.isArray(schema.required)
        ? schema.required.filter((f) => typeof f === "string")
        : [];
}
function removeFromRequired(inputSchema, fields) {
    if (!Array.isArray(inputSchema.required))
        return;
    const remove = new Set(fields);
    inputSchema.required = inputSchema.required.filter((f) => typeof f !== "string" || !remove.has(f));
}
function addToRequired(inputSchema, fields) {
    if (fields.length === 0)
        return;
    const current = Array.isArray(inputSchema.required) ? inputSchema.required : [];
    const merged = new Set();
...
function augmentAlternateFields(inputSchema, alternates) {
...
function extensionValueLines(key, value) {
    if (Array.isArray(value)) {
