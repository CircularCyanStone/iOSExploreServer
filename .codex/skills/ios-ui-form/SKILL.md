...
登录页这类 `UIScrollView` 包裹的表单不要把 `maxDepth` 设得太浅。`accessibilityIdentifierPrefix:"login_"` 只筛选 full 节点,不会自动深入遍历;若控件在第 6 层左右,`maxDepth:3` 或 `5` 可能只返回 `login_scroll_view`。登录/注册表单默认用 `maxDepth:8`,或不传 `maxDepth`。
