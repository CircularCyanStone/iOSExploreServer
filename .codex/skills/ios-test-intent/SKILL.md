...
description: |
  ...
  fail_criteria expressed in the ios-* observation vocabulary (textExists /
  targetExists / targetGone, plus alert observations). Native wait criteria are
  ready for ui_waitAny.conditions; alert is resolved through ui.inspect. Emits NO
  path / identifier / viewSnapshotID (those are resolved at runtime by ui.inspect).
...
离线读 iOS App 的业务源码,产出"测试意图 + 成功/失败判据清单"。原生等待判据用 `textExists` / `targetExists` / `targetGone` 表达,由执行层翻译为 `ui_waitAny.conditions`;`alert` 是独立观察判据,由执行层检查 `ui_inspect.alert`。本 skill 严格只产出**意图层**(测什么、什么算成败),**绝不产出执行层**(不写 `path` / `accessibilityIdentifier` / `viewSnapshotID`,那些由运行时 `ui_inspect` 现场解析)。
...
| `alert` | 弹窗出现；`value` 优先记录稳定标题/消息,无稳定文本时为 null | 不进入 waitAny；运行时检查 `ui_inspect.alert` |
...
`Criterion` 子对象:`mode`(textExists / targetExists / targetGone / alert) / `value`(可见文案或角色标签,alert 无稳定文案时 null) / `description` / `timing`。

需要产出完整 JSON 时读取 [references/intent-contract-example.md](references/intent-contract-example.md)。该 reference 只提供占位模板,不改变这里的字段语义。
