...
真机的 38321 端口不暴露给 Mac，必须经 `iproxy` USB 隧道转发。本 skill 提供一键管理脚本 `$SKILL_DIR/scripts/iproxy-manager.sh`，自动处理安装、launchd 托管启动、端口清理、设备检测。

**脚本定位硬规则**：`SKILL_DIR` 是本 `ios-connection/SKILL.md` 所在目录，不是当前仓库根目录、shell 当前目录或目标 App 工程目录。Agent 执行脚本前必须先用已展开的 `SKILL.md` 路径计算 `SKILL_DIR`，再运行 `"$SKILL_DIR/scripts/iproxy-manager.sh" <command>`。禁止直接从当前工作目录运行 `scripts/iproxy-manager.sh <command>`；那会把 skill 内相对路径误解析成项目根路径。
...
3. **`build_run_*` 不注入 session env** — 要传启动参数（如回到流程起点），必须用 `launch_app_*(env/launchArgs)`，且先 `stop_app_*` 再 `launch_app_*`（已运行的 App 不会重启、参数不生效）。同一个测试入口优先只用 `launchArgs`；不要同时传语义重复的 `env` 和 `launchArgs`，除非宿主 App 明确要求两套开关都存在。
4. **真机前先确认 38321 是 `iproxy` 在监听** — 模拟器跑过的 App 可能残留成 Mac 进程占住 38321，导致真机预期对不上。Agent 用 `health_check` 验证连接；开发者手动排查时用 `"$SKILL_DIR/scripts/iproxy-manager.sh" status` 检查占用进程类型。
...
以下操作**仅供开发者在终端手动使用**，Agent 优先使用 MCP 工具做连接验证；连接失败且需要本机端口诊断时，应提示用户手动执行 `"$SKILL_DIR/scripts/iproxy-manager.sh" status`。
...
```bash
"$SKILL_DIR/scripts/iproxy-manager.sh" status
# 自动检查：
...
- **开发者手动修复**：`"$SKILL_DIR/scripts/iproxy-manager.sh" restart`（自动卸载旧 LaunchAgent / 停止旧 iproxy → 重建 launchd 托管 → 验证；若端口被模拟器 App 占用，先按脚本提示执行 `clean`）
...
- **开发者手动修复**：`"$SKILL_DIR/scripts/iproxy-manager.sh" restart`
...
- **Agent 处理**：先 `stop_app_*` 再 `launch_app_*(launchArgs=[...])`。若启动开关同时支持环境变量和命令行参数,优先选一种最稳定的形式（通常是 `launchArgs`），避免同一语义双传导致报告难以判断哪个开关生效。
