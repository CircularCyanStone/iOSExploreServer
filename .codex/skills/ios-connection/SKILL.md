...
**不做**：不处理 App 已连上后的 UI 操作（回到 `ios-automation` 路由到 `ios-ui-*`），不构建/安装 App（走 L0 `ios-debugger-agent`）。
...
- ❌ 需要构建/安装/LLDB 调试 → 走 L0 `ios-debugger-agent`
...
1. **启动 iproxy（开发者手动）** — iproxy 必须由**开发者在终端手动执行** `scripts/proxy.sh` 或 `scripts/proxy.sh --daemon`（后台运行）启动。Agent 无法代劳启动 iproxy（没有对应的 MCP 工具），只能调用 `mcp__iOSDriver__health_check` 验证 iproxy 是否已启动；未启动时提示用户手动执行上述命令。
...
`scripts/proxy.sh` 自动处理：iproxy 安装检查、残留清理、UDID 获取、状态诊断。Agent 的职责是验证连接和引导用户，不能直接执行脚本。
...
2. **检查 App 是否运行** — 真机场景提示用户在终端执行 `scripts/proxy.sh --status` 检查 iproxy 状态和端口占用
3. **尝试启动 App** — 调用 `mcp__XcodeBuildMCP__launch_app_device` 或 `launch_app_sim`，观察启动结果

### 开发者手动排查（仅供终端使用，Agent 禁用）

以下操作**仅供开发者在终端手动使用**，Agent 禁止调用任何 bash 命令或脚本。Agent 只能使用 MCP 工具，连接失败时应提示用户手动执行 `scripts/proxy.sh --status` 进行诊断。
...
# 自动检查：
# - 端口 38321 占用情况（是 iproxy 还是残留 SPMExample 进程）
# - USB 设备连接状态（真机场景）
# - iproxy 服务可用性
# - 并给出针对性修复建议
```
...
- **开发者手动修复**：`scripts/proxy.sh --status` 检查端口和服务状态
...
- `ios-debugger-agent`（L0） — App 未安装或需构建时改用它
