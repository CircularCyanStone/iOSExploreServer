# 本地临时端到端测试：不安装 MCPServer 到任何客户端

开发期间改了 MCPServer 后想立刻验证 MCP 工具（`ui_inspect` / `ui_tap` 等）的端到端效果，**不需要把 MCPServer 注册到 Claude Code / Cursor / Codex 等 MCP 客户端**，也**不需要 npm link / npm publish**。仓库自带 `scripts/mcp-inspector.mjs`，它扮演一个最小 MCP 客户端：通过 stdio 跟 MCPServer 子进程走 JSON-RPC 2.0 通信，再把每个工具调用的响应原样打到 stdout。
...
1. MCPServer 已编译：`npm run build`（产物在 `dist/`）。
...
- `npm test`（vitest）覆盖 MCPServer 内部逻辑：工具名映射、schema 转换、动态工具发现、transport 重试、screenshot 转 image content 等，**mock 掉真实 HTTP**，不需要真 App 跑。
- `scripts/mcp-inspector.mjs` 走真 stdio + 真 HTTP，验证 MCPServer 在真 MCP 协议下、真 App 响应下的端到端表现。两者互补：改完内部逻辑先 `npm test`，再起 App 走 mcp-inspector 做端到端 smoke。
