...
describe("IOSExploreClient", () => {
    test("posts action and data to POST /", async () => {
        await withMockIOSExploreServer(request => ({ body: { code: "ok", data: { echoed: request.data } } }), async ({ baseURL, requests }) => {
    ...
    test("throws structured iOS envelope error", async () => {
        await withMockIOSExploreServer(() => ({ body: { code: "invalid_data", message: "bad field" } }), async ({ baseURL }) => {
            const client = new IOSExploreClient({ baseURL, requestTimeoutMs: 10000 });
    ...
    test("throws structured HTTP error with body snippet", async () => {
        await withMockIOSExploreServer(() => ({ status: 500, body: "server exploded" }), async ({ baseURL }) => {
            const client = new IOSExploreClient({ baseURL, requestTimeoutMs: 10000 });
    ...
    test("wait action timeout uses command timeout plus grace", async () => {
        await withMockIOSExploreServer(() => ({ delayMs: 50, body: { code: "wait_timeout", message: "timeout" } }), async ({ baseURL }) => {
