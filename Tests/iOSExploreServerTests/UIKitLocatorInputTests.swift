...
func uikitLocatorInputParsesIdentifierTarget() throws {
    let decoder = CommandInputDecoder(
        ...
        definition: CommandInputDefinition(fields: [
...
func parseOptionalLocator() throws {
    let definition = CommandInputDefinition(fields: [
        UIKitLocatorFields.accessibilityIdentifier.erased,
        UIKitLocatorFields.path.erased,
    ])
    // 都缺 → nil
    var d1 = CommandInputDecoder(JSON([:]), definition: definition)
    #expect(try UIKitLocatorInput.parseOptional(decoder: &d1) == nil)
    // 单 identifier → 解析成功
    var d2 = CommandInputDecoder(JSON(["accessibilityIdentifier": "home.submit"]), definition: definition)
    ...
    // 单 path → 解析成功
    var d3 = CommandInputDecoder(JSON(["path": "root/0"]), definition: definition)
    let pathTarget = try UIKitLocatorInput.parseOptional(decoder: &d3)
    #expect(pathTarget != nil)
    // 两者都给 → 互斥抛错
    #expect(throws: CommandInputParseError.self) {
        var d4 = CommandInputDecoder(JSON(["accessibilityIdentifier": "x", "path": "root/0"]), definition: definition)
