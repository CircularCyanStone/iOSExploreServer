...
读源码用 `Read` / `Glob` / `Grep` 三个内置工具(可选:`codegraph_explore` 加速符号定位,需项目已建索引)。**不调用任何 `mcp__iOSDriver__*` 工具,不连 App、不操作 UI**。
...
**5 步工作流**,每步说清"读什么、提取什么、产出什么"。所有举例用占位 App `<MyApp>`(虚构),不引任何真实项目类名/行号。完整真实 App 套用案例见"相关 skill"末尾的 examples 指针。
...
**完整真实 App 套用案例**:SPMExample 登录流程的完整意图清单 + 实跑报告作为"如何对真实 App 套用 test-intent / runner"的参考案例,位于 `docs/skills/examples/spmexample-login/`(由 plan Task 15 迁移后;迁移前临时位于 `docs/test-intents/spmexample-login.json` 与 `docs/test-reports/`)。这是本仓库内唯一的真实 App 例子,skill 本体不依赖它。
...
- skill 自测用例:`evals/evals.json`(静态结构 case + 占位 App 方法 case)。
