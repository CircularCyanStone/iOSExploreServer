...
2. **次选 cell 内标题文本**：cell 里的 `UILabel` 有标题文本（如"📋  日志诊断测试"），但 `ui.viewTargets` 默认不返回 UILabel 这种纯文本节点。两种办法：
   - 调 `ui.topViewHierarchy`，钻到 cell 内部 `root/5/0/1/0` 这种**四级子节点**读 `text.value`，确认这是哪个菜单项，**但**然后用 cell 的 path (`root/5/0/1`) 去点击。
   - 调 `ui.viewTargets` 时设 `includeStaticText=true`（如果库版本支持），把标题文本带出来直接选。
3. **`ui.tap` 返回里的 `indexPath` 是唯一权威**：你点完一个 cell，响应里有 `indexPath: {item, section}`。把它和 menuItems 数组对应就能 1:1 确认你点的是哪一项。**前提是你得先点一次** —— 这就是为什么"先 dryRun、后真点"或"先查清楚再点"对 cell 场景特别重要。
4. **section + item 显式定位**：`ui.scrollToElement` + `match: "text"` 可以把目标 cell 滚到可见，但仍需先确认文本。

### 2.4 MCP / Skill 层的待办（库改进）

库目前在 cell 场景下确实**没给调用方足够信息**，建议后续改进（按价值排序）：

1. **`ui.topViewHierarchy` 给 `UITableViewCell` / `UICollectionViewCell` 节点加 `indexPath: {item, section}` 字段**（最关键）。调用方读层次时直接拿到 indexPath，不用靠 subviews 顺序或 y 排序猜。**这一条能根治本次案例的反复出错**。
2. **`ui.viewTargets` 给 cell / list 类节点带 `primaryText` 或 `cellTitle`**（次重要）。无 `accessibilityIdentifier` 的 cell 也能在 viewTargets 里一眼区分，免得调用方钻 topViewHierarchy 四级子节点。
3. **`ui.tap` 的 `description` 显式写明"调用前必须先 `ui.viewTargets`，并把同响应的 `viewSnapshotID` 原样传入"**，或提供一步到位的 `ui.tapByPath`（内部自取快照），降低两步调用契约的丢失概率。

> 这三条已记入 [docs/superpowers/specs/2026-07-05-uitableviewcell-tap-selection-design.md](../superpowers/specs/2026-07-05-uitableviewcell-tap-selection-design.md) 跟进。
...
```
# 1) 先调 topViewHierarchy 钻到 cell 内部读标题，确认是哪个 cell
curl -X POST http://localhost:38321/ -d '{"action":"ui.topViewHierarchy","data":{"maxDepth":25}}'
#  → 在响应里找到 cell path，比如 root/5/0/1，钻到 root/5/0/1/0 读 text.value="📋  日志诊断测试"

# 2) 调 viewTargets 拿 viewSnapshotID
curl -X POST http://localhost:38321/ -d '{"action":"ui.viewTargets","data":{}}'
#  → 响应 data.viewSnapshotID = "snap-9"

# 3) 用 cell 的 path + viewSnapshotID 点击
curl -X POST http://localhost:38321/ -d '{
  "action":"ui.tap",
  "data":{"path":"root/5/0/1","viewSnapshotID":"snap-9"}
}'
#  → 响应 data.indexPath = {item:2, section:0}；用 indexPath 校验你点对了
```
