...
`ui.*` 命令不是孤立的 HTTP 接口，它们之间有**严格的前置/后置关系**。最常见的错误模式是：跳过前置命令、直接发交互命令（`ui.tap` / `ui.control.sendAction` / `ui.input` / `ui.scroll`），结果因为缺少 `viewSnapshotID`、定位字段或新鲜 UI 结构而报错，然后开始盲试参数。
...
| 你要做什么 | 用哪个 | 原因 |
|---|---|---|
| 要执行 `ui.tap` / `ui.control.sendAction`，或给 `ui.input` / `ui.scroll` 做陈旧校验 | `ui.inspect` | 它签发 `viewSnapshotID`；`topViewHierarchy` 不签发，不能直接接 tap/control/freshness |
...
MCP 调用方使用 iOSDriver 的静态 `ui_inspect`、`ui_waitAny`、`wait_and_inspect` 等工具；宿主私有、Debug 或实验性 action 才通过 `call_action` 转发。App `help` 只做能力检查，不生成或刷新 MCP 工具。
...
| `ui.input` | `ui.inspect`（推荐） | 顶层 `fields[]` + 可选 `viewSnapshotID`；每个 field 带 `text` + (`path` 或 `accessibilityIdentifier`) | 否（仅文本） |
...
> 这部分行为已经收敛为当前 `ui.inspect` / `ui.topViewHierarchy` / `ui.tap` 的正式契约，不再依赖单独的历史设计稿。
