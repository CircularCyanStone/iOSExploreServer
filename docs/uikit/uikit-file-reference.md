...
- **关键点**：`UIInspectCollector` 与 executor 共用本类型生成/复核 `availableActions`。disabled 或祖先不可交互时返回空集合。规则：`UITextField` → input + 编辑三件套；值型控件 → `valueChanged`；其余 control → touchDown/UpInside。tap 来自 `UIKitDefaultActivationResolver`、cell selection，或 Debug 下确实能读取 target-action 的 gesture adapter；UISlider/UISegmentedControl 无 tap 语义。cell 子树 view 声明 `.tap`，由 `didSelectRow/didSelectItem` 路径派发。
...
- **关键点**：**通用 snapshot 校验与 tap/control 执行核心**。固定流程：取 Context → resolve locator → 验证 snapshot 为目标签发了请求动作 → 校验 context/fingerprint freshness → 执行动作。tap、精确 control event 在本文件传入 `requiredAction`；input 与定向 scroll 复用同一 validator，分别传 `.input` / `.scroll`。有效 snapshot 缺少动作返回 `not_actionable`；未知/过期 snapshot 或指纹变化返回 `stale_locator`。
...
- **关键点**：V1 路由表：`UIButton` → `sendActions(.touchUpInside)`；`UISwitch` → 翻转 + `.valueChanged`；`UITextField`/`UITextView` → `becomeFirstResponder()`。`UISlider`/`UISegmentedControl`/无手势的普通 `UIView` 无默认激活路由。挂有可执行 target-action 的非 UIControl gesture view 由 `UIGestureTargetExecutor` 处理，并在 Debug inspect 中声明 `tap`；navigationBar 和 alert 仍走专用命令。
...
- **关键点**：仅 `ui.inspect` 签发 `viewSnapshotID`。每个 path 保存 `UIKitSnapshotTarget(fingerprint, availableActions)`；`isActionSigned` 区分动作未授权的 `not_actionable`，`isStale` 区分未知/过期/context 或 fingerprint 变化的 `stale_locator`。容量 **32 条快照 × 每条最多 512 个 target**，单 path TTL **120 秒**，whole-table `snapshotChanged` TTL **300 秒**，淘汰策略“先过期后 LRU”。公开 `insert` 接收 `[String: UIKitSnapshotTarget]`，不保留无法推导动作授权的 fingerprint-only overload。
...
- **关键点**：`UIInspectInput` conform core `CommandInput`，字段定义同时驱动解析和 schema；`isFull` 按 control、scroll view、gesture、静态文本和 accessibility 语义把节点分成 full/minimal。full 节点全部签发 fingerprint + `availableActions`，所以静态 `UILabel` 等观察节点也可以是 full，但动作集合为空；minimal 节点只输出 `{path, type}` 且不签发。按钮内部 label/image 汇总到父 control。disabled control 保留在响应中但 `availableActions=[]`。`maxTargets` 默认 200（上限 512，仅计 full），identifier 完整不裁剪。
...
- **关键点**：collector 为最终返回的每个 full target 保存 `UIKitSnapshotTarget`，其中 fingerprint 和 `availableActions` 与同一响应同源。**不变式**：full returned paths 等于 snapshot target paths；可执行动作则逐 path 等于其 `availableActions`，不再把“已签发 path”等同于“可执行任意动作”。minimal path 或未声明动作返回 `not_actionable`。响应字段是顶层 `viewSnapshotID`。
...
- **`UIAlertRespondExecutor.swift`** 🍎 — `@MainActor`。按 `buttonTitle`/`buttonIndex`/`role` 选按钮（都不传时单按钮 alert 默认点、多按钮抛 alertButtonRequired），调 `UIAlertController.explore_dismissWithAction(_:)`（系统私有 `_dismissWithAction:`，由系统自动 dismiss + 调 handler，嵌套 present 也由系统协调），返回 `{ performed, dismissed, button }`；未 present 的 alert（典型是 logic test 构造对象）回退 `UIAlertAction.explore_performHandler()` 直接调 handler block（`dismissed=false`）。Release 下回退 `alertRespondDisabledInRelease` / `alert_release_unsupported`（私有 API 被 `#if DEBUG` 隔离，补按钮参数也不能启用触发）。查询 alert 结构（标题/按钮/输入框）不在此命令——走 `ui.inspect` 的 alert 区块（含 path/availableActions，更全）。
- **`UIGestureTargetExecutor.swift`** 🍎 — `@MainActor`。`supportsTap(on:)` 仅在 Debug runtime 能读到至少一个 target-action 时让 inspect 声明 `tap`；`execute(on:)` 用同一读取逻辑按 selector 签名派发，多手势/多 target 全触发。weak target 已释放时读取结果为空，目标不会被签发 tap。UIControl 被排除，避免触发 slider 等内部手势。cell selection adapter 同居此文件，并优先使用 `indexPath(for:) + delegate.didSelectRow/didSelectItem` 的公开路径。
- **`UIKitInternalUtils.swift`** 🍎 — `@MainActor`。`UIView` 扩展 `explore_cellAncestor` / `explore_containerViewAncestor`：前者向上找首个 `UITableViewCell`/`UICollectionViewCell` 祖先，后者向上找首个 `UITableView`/`UICollectionView` 祖先。供 `UIGestureTargetExecutor.executeCellSelection` 与 `UIKitActionCapabilityResolver.resolve` 共享——前者用于 cell 子树判定 + cell/container 摘要，后者用于声明 cell 子树 view 的 `.tap` 能力（agent 不必靠 `hasGestureRecognizers` 蒙）。整体 `#if canImport(UIKit)`（macOS SPM 空壳）。
...
| 主题 | 文档 |
|---|---|
| 模块整体架构（typed factory、隔离边界） | `docs/architecture/index.md` |
| UIKit 阅读路线 | `docs/uikit/reading-guide.md` |
| agent 调用契约 | `docs/uikit/agent-command-protocol.md` |
