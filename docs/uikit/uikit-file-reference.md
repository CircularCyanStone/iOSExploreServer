...
| `Snapshot/` | 3 | 陈旧检测（指纹快照） |
| `Tap/` | 2 | `ui.tap` 命令 |
| `ControlAction/` | 2 | `ui.control.sendAction` 命令 |
| `ViewHierarchy/` | 3 | `ui.topViewHierarchy` 命令 |
| `ViewTargets/` | 3 | `ui.viewTargets` 命令 |
...
- **职责**：UIKit 命令失败的统一错误工厂（包装 core `ExploreCommandFailure`）。
- **关键点**：所有失败出口走这里，不在调用点散写 code/message/logMessage。错误码语义：定位/命中类失败 → `.invalidData`；UIKit 上下文不可用 → `.internalError`。
...
- **关键点**：是 UIKit 命令进入 MainActor 隔离域的第一个入口。`Context` 持有 UIKit 对象**不可跨 MainActor 边界传递**。`currentContext()` 在 window/rootVC/view 不可用时返回原因。`topViewController` 递归穿透 presentedVC / nav / tab / split。
- **依赖**：UIKit。
...
- **关键点**：`locate` 返回 `found/notFound/ambiguous`；identifier 匹配多个返回 `ambiguous`；还提供 `nearestControl`（向上找最近 UIControl）与祖先关系判断。`windowPoint` 不在此解析（交给 executor hit-test）。
- **依赖**：UIKit、`UIKitLocator`、`UIKitViewLookupTarget.pathString`。
- **被调用**：executor。
...
- **关键点**：**全模块执行核心**。固定流程：取 Context → resolve locator → 陈旧校验（仅 `.path + snapshotID`）→ 能力校验 → hit-test（tap）/ `sendActions(for:)`（control）。复用调用方已 locate 的 `LocatedView` 避免二次遍历。有 `execute(_:context:)` 注入入口供测试。
...
- **关键点**：查询命令签发 snapshotID 返回给调用方；交互命令携带它时，executor 执行前比对指纹，不匹配返回 `invalid_data`（固定消息 "locator is stale; re-query"）。容量 **8 条快照 × 每条最多 512 指纹**，TTL **10 秒**，淘汰策略"先过期后 LRU"。时间可注入（`setNow`），测试推进时间即可触发过期。
- **依赖**：`UIKitCommandLogging`。
- **被调用**：两个 collector（insert）、executor（validation）。
...
- **关键点**：薄 adapter——解析请求 → 构造 `UIKitActionPlan.tap` → `await executor.execute(plan)`。执行逻辑全在 executor。
- **依赖**：`UITapQuery`、`UIKitActionPlan`、`UIKitActionExecutor`、`UIKitCommandError`、`UIKitCommandLogging`。

---

## `ControlAction/`（`ui.control.sendAction`）
...
- **关键点**：薄 adapter——解析请求 → 构造 `UIKitActionPlan.controlEvent` → `await executor.execute(plan)`。
- **依赖**：`UIControlSendActionQuery`、`UIKitActionPlan`、`UIKitActionExecutor`、`UIKitCommandError`、`UIKitCommandLogging`。

---

## `ViewHierarchy/`（`ui.topViewHierarchy`）

### `UIViewHierarchyModels.swift` ✅（651 行，最大文件）
...
- **关键点**：`UIKitViewElement` 是 `UIViewHierarchyElement` 的 UIKit 实现；读 UIKit 后交给 Foundation-only 的 `UIViewHierarchyBuilder`。还签发 snapshot（含完整树指纹）。
...
- **关键点**：薄 adapter——解析 `UIViewHierarchyQuery` → 调 collector。有 identifier 筛选时返回 `matches` 列表，否则返回完整 `root` 树。
- **依赖**：`UIViewHierarchyQuery`、`UIViewHierarchyCollector`、`UIKitCommandError`、`UIKitCommandLogging`。

---

## `ViewTargets/`（`ui.viewTargets`）

### `UIViewTargetsModels.swift` ✅（381 行）
...
- **关键点**：刻意不复用完整层级快照（不读颜色/字体/图片）。identifier 筛选不提前剪枝子树。**`availableActions` 只认目标自身的 control 身份**（不向上借祖先 control），避免与 executor 的 path 派发分叉。有 `collect(query:context:)` 注入入口。
...
- **关键点**：薄 adapter——解析 `UIViewTargetsQuery` → 调 collector。响应含 `targetCount`/`visitedNodeCount`/`truncated`/`snapshotID`。
