...
**Tech Stack:** Swift 6.2（iOSExploreServer core + iOSExploreUIKit）、Swift Testing（`@Test`）、TypeScript（iOSDriver stdio）、XcodeBuildMCP（端到端验证）。
...
| `iOSDriver/src/staticTools.ts` | MCP 静态工具 | 废弃 observe、wait_and_observe→wait_and_inspect、viewTargetsOptions→inspectOptions |
...
Run: `grep -rn "ui\.viewTargets\|viewTargets" Sources/ Tests/ iOSDriver/src/ Examples/ docs/ | grep -v "\.git"` 记录所有命中。
...
### Task 10: iOSDriver staticTools——废弃 observe、wait_and_inspect、inspectOptions

**Files:**
- Modify: `iOSDriver/src/staticTools.ts`（observe L82-92、wait_and_observe L93-112、schema L128-162）
- Modify: `iOSDriver/tests/staticTools.test.ts`
- Build: `cd iOSDriver && npm run build`
...
Run: `cd iOSDriver && npm test -- staticTools`
Expected: FAIL
...
```bash
git add iOSDriver/src/staticTools.ts iOSDriver/src/server.ts iOSDriver/tests/
...
- Modify: `iOSDriver/README.md`、`iOSDriver/docs/local-mcp-test.md`（工具名映射表、示例）
...
用 `iOSDriver/scripts/mcp-inspector.mjs`（profile `sim-app`，App 以 `IOS_EXPLORE_AUTOSTART=1` 启动）：
...
```bash
git add AGENTS.md docs/ iOSDriver/README.md iOSDriver/docs/
