...
- **测试方式**：用 `scripts/mcp-inspector.mjs` + `curl` 走真实 MCP 协议 → iOSDriver → SPMExample（iPhone 17 模拟器，38321 端口，autostart）。自行构造场景（新增 `ControllerStructureTestViewController`，覆盖 Navigation / Tab / Modal 链 / Child / Split 五类容器结构），不依赖既有页面。
